...
function Button({ toggled = false }) {
  const { ref, isToggled } = useHeadlessSignals(
...
function Password() {
  const { visible, inputType, toggle } = useHeadlessSignals(() =>
    showPassword(),
  );
...
function DragAndDrop() {
  ...
  return html`
    ...
    <div ref=${droppableRef} style="border: 1px solid black; padding: 5px;">
      Dropzone ${inDropzone.value && draggableEl}
    </div>
  `;
}
...
function MediaQuery() {
  ...
  return html` ${useComputed(() => (matches.value ? '> 600px' : '<= 600px'))} `;
}
...
if (root) {
  render(
    html`
      ...
      <div>
        <${ReorderableList}
          items=${['foo', 'bar', 'lorem', 'ipsum', 'dolor']}
        />
      </div>
      <div>
        <${DragAndDrop} />
