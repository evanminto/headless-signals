...
export function toggleButton(initialIsToggled = false) {
  const { ref: buttonRef } = eventListener('click', () => isToggled.value = !isToggled.value);
  const isToggled = signal(initialIsToggled);
  const ariaPressed = computed(() => isToggled.value ? 'true' : 'false');
  const el = computed(() => buttonRef.current instanceof HTMLButtonElement ? buttonRef.current : null);
  const dispose = effect(() => el.value?.setAttribute('aria-pressed', ariaPressed.value));
